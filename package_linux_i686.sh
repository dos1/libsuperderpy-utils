...
if [ -z "$NO_STEAM_RUNTIME" ]; then
  ...
  schroot --chroot steamrt_scout_i386 -- /bin/sh -c 'HOME=/tmp/steam-chroot-home cmake ../.. -DCMAKE_BUILD_TYPE=RelWithDebInfo  -DUSE_CLANG_TIDY=no -DCMAKE_C_FLAGS="--std=c99 -fPIC" -DCMAKE_CXX_FLAGS="-fPIC"'
